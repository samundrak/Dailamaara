...
import np.com.samundrakc.game.controllers.subControllers.Query;
    ...
    public void dragStop(InputEvent event, float x, float y, int pointer) {
        ...
        if (player.getGame().getPLAY_TURN().getId() == player.getId() && (player.getGame().getPLAY_TURN() != null)) {
            if (player.getGame().getHistory().size() < 1) {
                ...
                player.getGame().getItihaas().add(new np.com.samundrakc.game.controllers.dailamaara.History());
            }
            ...
            player.getGame().getItihaas().get(player.getGame().getItihaas().size() - 1).addPlayerWithCards(new np.com.samundrakc.game.controllers.dailamaara.History.PlayerHistory(card, player));
            player.getGame().selectPlayOfTurup(player, card);
            player.setThrownCard(card);
            player.doExtraStuff();
            card.getActor().clearListeners();
            card.getActor().setZIndex(0);
